...
You can run the prebuilt version instantly from the `docs/` folder (no installs), or use the Vite dev server for local development.

## Quick start

### Fastest: open the static build (no installs)

1. Open `docs/index.html` directly in your browser, or serve the `docs/` directory with any static server:
   ```bash
   cd docs
...
1. If you change the React source under `src/`, run `npm run build` to regenerate the `docs/` folder. Otherwise, the checked-in `docs/` folder is already ready to deploy.
